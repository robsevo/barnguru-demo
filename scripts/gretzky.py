...
COMMANDS: list[Cmd] = [
    ...
    Cmd("sync-coach-images", "phase1",
        "Populate image_url for every head coach in data/coaches.json via Wikipedia",
        "scripts.sync_coach_images"),
    ...
    Cmd("train-matchup-playoffs", "phase2",
        "Matchup Matrix — playoffs variant",
        "scripts.train_matchup_model", args=["--season-type", "playoffs"]),
    Cmd("train-chemistry", "phase2", "Train Line Chemistry Model — linemate pair xGF% (Feature 2.4)",
        "scripts.train_chemistry_model"),
    Cmd("train-chemistry-playoffs", "phase2",
        "Line Chemistry — playoffs variant",
        "scripts.train_chemistry_model", args=["--season-type", "playoffs"]),
    ...
    Cmd("train-bayesian-playoffs", "phase2",
        "Bayesian ratings — playoffs variant",
        "scripts.train_bayesian_ratings", args=["--season-type", "playoffs"]),
    ...
    Cmd("compute-heatmap-playoffs", "phase2",
        "Positional Heatmap — playoffs variant",
        "scripts.compute_positional_heatmap", args=["--season-type", "playoffs"]),
    ...
    Cmd("train-deployment", "phase4",
        "Train Line Deployment Forecaster — F lines + D pairs + minutes (Feature 4.1)",
        "scripts.train_line_deployment"),
    Cmd("train-matching", "phase4",
        "Train Line-Matching Model — defensive counter-deployment (Feature 4.2)",
        "scripts.train_line_matching"),
    Cmd("train-st-deployment", "phase4",
        "Train Special Teams Deployment — PP1/PP2/PK1/PK2 personnel (Feature 4.3)",
        "scripts.train_st_deployment"),
    Cmd("train-timeout", "phase4",
        "Train Timeout Usage Model — P(timeout) by (period, score, time) per coach (Feature 4.4)",
        "scripts.train_timeout_usage"),
    Cmd("train-goalie-pull", "phase4",
        "Train Goalie Pull Timing Model — P(pull) by (deficit, time remaining) per coach (Feature 4.5)",
        "scripts.train_goalie_pull"),
    Cmd("train-penalty-tendency", "phase4",
        "Train Penalty Tendency Model — per-team foul + PP-opportunity rates (Feature 4.6)",
        "scripts.train_penalty_tendency"),
    Cmd("train-coach-profile", "phase4",
        "Build per-current-head-coach career-with-team tendencies (Feature 4.7)",
        "scripts.train_coach_profile"),
    Cmd("train-goalie-coach", "phase4",
        "Train Goalie Coach Curve — team save% trajectory + change-point detection (Feature 4.8)",
        "scripts.train_goalie_coach"),
    Cmd("train-pp-coordinator", "phase4",
        "Train PP Coordinator system signature — xG/60, carry%, PP1 QB (Feature 4.9)",
        "scripts.train_pp_coordinator"),
    Cmd("train-pk-coordinator", "phase4",
        "Train PK Coordinator system signature — SV%, xGA/60, SH pressure (Feature 4.10)",
        "scripts.train_pk_coordinator"),
    Cmd("train-coaching-style", "phase4",
        "Train 8-dim Coaching Style Vector from PBP (Feature 4.11)",
        "scripts.train_coaching_style"),
    Cmd("train-roster-fit", "phase4",
        "Train Roster Fit Score — coach style × roster archetype composition (Feature 4.12)",
        "scripts.train_roster_fit"),
    Cmd("detect-staff-changes", "phase4",
        "Detect mid-season coaching staff changes from transactions (Feature 4.13)",
        "scripts.detect_staff_changes"),
    Cmd("detect-fo-regime", "phase4",
        "Detect front office regime changes — GM/AGM/Pres (Feature 4.14)",
        "scripts.detect_fo_regime"),
    Cmd("classify-buyer-seller", "phase4",
        "Classify trade deadline buyer/seller/neutral per team (Feature 4.15)",
        "scripts.classify_buyer_seller"),
    Cmd("seller-motivation", "phase4",
        "Compute post-deadline seller motivation drag per team (Feature 4.16)",
        "scripts.compute_seller_motivation"),
    Cmd("train-decision-net", "phase4",
        "Train per-coach decision network from Phase 4 outputs (Feature 4.17)",
        "scripts.train_coach_decision_net"),
    Cmd("gm-fingerprint", "phase4",
        "Compute per-GM behavioral fingerprint — action archetypes (Feature 4.18)",
        "scripts.compute_gm_fingerprint"),
    Cmd("venue-atmosphere", "phase4",
        "Compute per-arena venue scare factor from PBP (Feature 4.19)",
        "scripts.compute_venue_atmosphere"),
    Cmd("playoff-elimination", "phase4",
        "Compute playoff elimination fatigue drag per team (Feature 4.20)",
        "scripts.compute_playoff_elimination"),
    Cmd("dow-signature", "phase4",
        "Compute per-player day-of-week signature + broadcast context (Feature 4.21)",
        "scripts.compute_dow_signature"),
