...
export default function TeamThemeLoader() {
  const { theme, isShowingLoader, isCortexLoading } = useTheme();
  const [progress, setProgress] = useState(0);
  const [visible, setVisible] = useState(false);
  const [exiting, setExiting] = useState(false);

  const activeLoader = isShowingLoader || isCortexLoading;
  ...
  // Cortex loader — no team theme
  if (isCortexLoading && !theme) {
    const color = "#a78bfa";
    return (
      <div
